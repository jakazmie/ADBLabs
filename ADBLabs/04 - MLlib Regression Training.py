...
# COMMAND ----------
  ...
  df = spark.read.format('csv').option("header", 'true').load("/databricks-datasets/bikeSharing/data-001/hour.csv")
...
# MAGIC 
# MAGIC df <- read.df("/databricks-datasets/bikeSharing/data-001/hour.csv", source="csv", header="true")
...
# MAGIC %r
# MAGIC 
# MAGIC print(c("Our dataset has ", count(df), " rows"))
...
# MAGIC %r
# MAGIC 
# MAGIC df = drop(df, c("instant", "dteday", "casual", "registered"))
# MAGIC display(df)
...
# MAGIC %r
# MAGIC 
# MAGIC df = select(df, lapply(columns(df), function(x) alias(cast(df[[x]], "double"), x)))
# MAGIC printSchema(df)
...
# MAGIC %r
# MAGIC 
# MAGIC df_list = randomSplit(df, c(0.7, 0.3), 0)
# MAGIC train = df_list[[1]]
# MAGIC test = df_list[[2]]
# MAGIC 
# MAGIC print(c("Training examples:", count(train)))
# MAGIC print(c("Testing examples:", count(test)))
...
# MAGIC 
# MAGIC We are going to save the datasets using `Parquet` format

# COMMAND ----------

test.write.mode("overwrite").parquet("/datasets/bike_test_data")
train.write.mode("overwrite").parquet("/datasets/bike_train_data")

# COMMAND ----------

# MAGIC %r
# MAGIC 
# MAGIC write.parquet(x=test, path="/datasets/bike_test_data", mode="overwrite")
# MAGIC write.parquet(x=train, path="/datasets/bike_train_data", mode="overwrite")
...
# MAGIC %r
# MAGIC 
# MAGIC features_cols = columns(df)
# MAGIC features_cols = features_cols[features_cols != "cnt"]
# MAGIC print(features_cols)

# COMMAND ----------

# MAGIC %md
# MAGIC 
# MAGIC So far we have been using *SparkR* to call Spark functions from R. *SparkR* is an "official" R interface to Spark. The alternative that has gained a lot of support is a community developed *sparklyr*. We will use *sparklyr* to configure and train Spark ML pipelines.
...
# MAGIC 
# MAGIC pipeline <- ml_pipeline(sc)
# MAGIC 
# MAGIC pipeline = ft_vector_assembler(x=pipeline, input_cols=features_cols, output_col="rawFeatures")
# MAGIC pipeline = ft_vector_indexer(x=pipeline, input_col="rawFeatures", output_col="features", max_categories=4)
...
# MAGIC %r
# MAGIC 
# MAGIC gbt = ml_gbt_regressor(sc, label_col="cnt")
...
# MAGIC %r
# MAGIC 
# MAGIC param_grid <- list(
# MAGIC     gbt_regressor = list(
# MAGIC         max_depth = c(2, 5),
# MAGIC         max_iter = c(10, 100)
# MAGIC     )
# MAGIC )
# MAGIC 
# MAGIC evaluator <- ml_regression_evaluator(x=sc, metric_name="rmse", label_col="cnt")
# MAGIC 
# MAGIC pipeline <- ml_cross_validator(x=pipeline, estimator=gbt, evaluator=evaluator, estimator_param_maps=param_grid)
...
train = spark.read.parquet("/datasets/bike_train_data").cache()
test = spark.read.parquet("/datasets/bike_test_data").cache()
...
# MAGIC library(dplyr)
# MAGIC 
# MAGIC if (length(src_tbls(sc)) > 0) {
# MAGIC       db_drop_table(sc, "train")
# MAGIC       db_drop_table(sc, "test")
# MAGIC }
# MAGIC 
# MAGIC train <- spark_read_parquet(sc=sc, name="train", path="/datasets/bike_train_data")
# MAGIC test <- spark_read_parquet(sc=sc, name="test", path="/datasets/bike_test_data")
# MAGIC 
# MAGIC pipeline_model = ml_fit(pipeline, train)
...
# MAGIC %r
# MAGIC 
# MAGIC predictions = ml_transform(pipeline_model, test)
...
# MAGIC %r
# MAGIC 
# MAGIC results <- predictions %>%
# MAGIC select("cnt", "prediction") %>%
# MAGIC glimpse()
...
# MAGIC %r
# MAGIC 
# MAGIC rmse = ml_evaluate(evaluator, predictions)
# MAGIC print(c("RMSE on out test set:", rmse))
